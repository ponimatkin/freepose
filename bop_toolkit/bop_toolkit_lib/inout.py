        ...
        with open(path, "r") as f:
            ...
            for line in f:
                ...
                if line_id == 1 and header in line:
                    ...
                    if len(elems) != 8:
                        raise ValueError(
                            "A line does not have 8 comma-sep. elements: {}".format(
                                line
                            )
                        )
                    ...
                    result = {
                        ...
                        "im_id": int(elems[1]),
                        # "obj_id": int(elems[2]),
                        "obj_id": elems[2],  # accept all strings as obj_id
                        "score": float(elems[3]),
                        "R": np.array(
...
def load_bop_results_bbox_visib(path, version="bop19"):
    """Loads 6D object pose estimates from a file.
    ...
    # See docs/bop_challenge_2019.md for details.
    if version == "bop19":
        header = "scene_id,im_id,obj_id,score,R,t,bbox_visib,scale,time"
        with open(path, "r") as f:
            line_id = 0
            for line in f:
                line_id += 1
                if line_id == 1 and header in line:
                    ...
                    if len(elems) != 9:
                        raise ValueError(
                            "A line does not have 9 comma-sep. elements: {}".format(
                                line
                            )
                        )
...
def load_mesh(path, scale=1.0):
    """Loads a 3D mesh model from a mesh file.

    HACK: by default, does not load texture (not needed for eval)

    :param path: Path to a .ply|.obj file.
    ...
    model['normals'] = mesh.vertex_normals
    if mesh.visual.kind == 'vertex':
        model['colors'] = mesh.visual.vertex_colors
    model['faces'] = mesh.faces
    # if mesh.visual.kind == 'texture' and texture_file_name is not None:
    #     assert texture_file_name is not None
    #     model['texture_uv'] = mesh.visual.uv
    #     # model['texture_uv_face'] = ?
    #     # Should be in mesh.visual.material.filename but empty
