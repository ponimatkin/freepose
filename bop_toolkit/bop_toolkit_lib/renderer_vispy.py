...
class RendererVispy(renderer.Renderer, app.Canvas, metaclass=SingletonArgs):
    ...
    def add_object_from_mesh(self, obj_id, model, scale=1.0, **kwargs):
        """See base class."""
        ...
        # model = inout.load_mesh(model_path, scale)
        self.models[obj_id] = {}
        self.models[obj_id]["pts"] = model["pts"].copy()
        self.models[obj_id]["faces"] = model["faces"].copy()
        self.models[obj_id]["normals"] = model["normals"].copy()

        if 'colors' in model:
            self.models[obj_id]["colors"] = model["colors"].copy()

        self.models[obj_id]['pts'] *= scale
    ...
    def add_object(self, obj_id, model_path, scale=1.0, **kwargs):
        """See base class."""
        # Color of the object model (the original color saved with the object model
        # will be used if None).
