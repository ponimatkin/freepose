...
p = {
    # Errors to calculate.
    "errors": [
        # WARNING: fails for meshes of different vertex number
        # {
        #     "n_top": -1,
        #     "type": "mssd",
...
for result_filename in p["result_filenames"]:
    ...
    ests = inout.load_bop_results_bbox_visib(
    ...
    if slurm_job_id == -1:
        ...
        # )
        print('AR over errors: ', list(average_recalls.keys()))
        final_scores["bop19_average_recall"] = np.mean(
            [e for e in  average_recalls.values()]
        )
