...
def vsd(
    ...
    :param inf_id: Object identifier of mesh used at inference time.
    :param gt_id: Object identifier of ground-truth mesh.    
    ...
    depth_est = renderer.render_object(inf_id, R_est, t_est, fx, fy, cx, cy)["depth"]
    depth_gt = renderer.render_object(gt_id, R_gt, t_gt, fx, fy, cx, cy)["depth"]
    ...
    # plt.figure('diff_depth_gt_est')
    # plt.imshow(dist_est - dist_gt)
    # plt.show()
...
def mssd(R_est, t_est, R_gt, t_gt, pts_e, pts_gt, syms):
    ...
    :param R_gt: 3x3 ndarray with the ground-truth rotation matrix.
    :param t_gt: 3x1 ndarray with the ground-truth translation vector.
    :param pts_e: nx3 ndarray with 3D model points which was used for estimation.
    :param pts_gt: nx3 ndarray with 3D model points used for groundtruth.
    ...
    """
    pts_est = misc.transform_pts_Rt(pts_e, R_est, t_est)
    es = []
    for sym in syms:
        R_gt_sym = R_gt.dot(sym["R"])
        t_gt_sym = R_gt.dot(sym["t"]) + t_gt
        pts_gt_sym = misc.transform_pts_Rt(pts_gt, R_gt_sym, t_gt_sym)
...
def chamfer(R_est, t_est, R_gt, t_gt, pts_e, pts_gt):
    """Chamfer distance on 3D vertices.
    ...
    """
    pts_est = misc.transform_pts_Rt(pts_e, R_est, t_est)
    pts_gt = misc.transform_pts_Rt(pts_gt, R_gt, t_gt)
    return chamfer_distance(pts_est, pts_gt)


def chamfer_proj(R_est, t_est, R_gt, t_gt, K, pts_e, pts_gt):
    """Chamfer distance on projected vertices.
    ...
    :return: The calculated error.
    """
    proj_est = misc.project_pts(pts_e, K, R_est, t_est)
    proj_gt = misc.project_pts(pts_gt, K, R_gt, t_gt)
    return chamfer_distance(proj_est, proj_gt)


def mspd(R_est, t_est, R_gt, t_gt, K, pts_e, pts_gt, syms):
    ...
    for sym in syms:
        R_gt_sym = R_gt.dot(sym["R"])
        t_gt_sym = R_gt.dot(sym["t"]) + t_gt
        proj_gt_sym = misc.project_pts(pts_gt, K, R_gt_sym, t_gt_sym)
...
def cus(R_est, t_est, R_gt, t_gt, K, renderer, inf_id, gt_id):
