...
p = {
    ...
    # Folder containing the mesh used at inference time 
    "models_inference_path": config.models_inference_path,
...
parser.add_argument("--models_inference_path", default=p["models_inference_path"])
...
p["models_inference_path"] = str(args.models_inference_path)
...
for result_filename in p["result_filenames"]:
    ...
    if not os.path.exists(p["models_inference_path"]):
        print(f'{os.path.exists(p["models_inference_path"])} does not exist')
    
    # HACK: loads also the visible bounding boxes to filter results
    # ests = inout.load_bop_results(os.path.join(p["results_path"], result_filename))
    ests = inout.load_bop_results_bbox_visib(os.path.join(p["results_path"], result_filename))
    # Get unique inference time object ids and path per scene
    # ((scene_id1, obj_id2), (scene_id1, obj_id3), (scene_id2, obj_id1), ...)
    scene_obj_ids_inf_tuple = set((est['scene_id'], est['obj_id']) for est in ests)
    scene_obj_ids_inf_dic = {}
    for scene_id, obj_id in scene_obj_ids_inf_tuple:
        if scene_id not in scene_obj_ids_inf_dic:
            scene_obj_ids_inf_dic[scene_id] = []
        scene_obj_ids_inf_dic[scene_id].append(obj_id)
    # inf_obj_ids_dis = {obj_id: f'inf_{obj_id}' for obj_id in inf_obj_ids}  # disambiguated ids
    inf_obj_path_tmpl = os.path.join(p["models_inference_path"], '{obj_id}', '{obj_id}.obj')


    # Load ground-truth object models.
    models = {}
    if p["error_type"] in ["ad", "add", "adi", "mssd", "mspd", "proj", "chamfer", "chamfer_proj"]:
        misc.log("Loading object models...")
        for obj_id in dp_model["obj_ids"]:
            models[obj_id] = inout.load_mesh(
    ...
    if p["error_type"] in ["vsd", "cus"]:
        ...
        for obj_id in dp_model["obj_ids"]:
            obj_path = dp_model["model_tpath"].format(obj_id=obj_id)
            ren.add_object(obj_id, obj_path)
    ...
    # get SLURM array job index
    slurm_array_job_index = int(os.environ.get('SLURM_ARRAY_TASK_ID', -1))

    for scene_ind, (scene_id, scene_targets) in enumerate(targets_org.items()):
        if slurm_array_job_index != -1 and scene_ind != slurm_array_job_index:
            continue
        else:
            misc.log(f"Processing scene {scene_id} by SLURM array job index {slurm_array_job_index}...")

        loaded_inf_ids = []
        unique_inf_scene_obj_ids = scene_obj_ids_inf_dic[scene_id]
        unique_inf_scene_obj_path = {inf_id: inf_obj_path_tmpl.format(obj_id=inf_id) 
                                     for inf_id in unique_inf_scene_obj_ids}
        
        # DONT LOAD ALL OBJECTS PER SCENE -> too long
        # # HACK: load object model per scenes
        # if p["error_type"] in ["ad", "add", "adi", "mssd", "mspd", "proj", "chamfer"]:
        #     misc.log(f"Scene {scene_id}, loading {len(unique_inf_scene_obj_ids)} inference meshes...")
        #     for i, inf_id in enumerate(unique_inf_scene_obj_ids):
        #         if i % 50 == 0:
        #             print(f' {i}/{len(unique_inf_scene_obj_ids)}')
        #         if inf_id in models:
        #             raise ValueError(f'Collision of object ids: {obj_id}')
        #         models[inf_id] = inout.load_mesh(
        #             inf_obj_path_tmpl.format(obj_id=inf_id)
        #         )

        # # HACK: load object model per scenes
        # if p["error_type"] in ["vsd", "cus"]:
        #     # retrieve unique object ids used at inference
        #     misc.log(f"Scene {scene_id}, loading {len(unique_inf_scene_obj_ids)} inference meshes in renderer...")
        #     for i, inf_id in enumerate(unique_inf_scene_obj_ids):
        #         if i % 50 == 0:
        #             print(f' {i}/{len(unique_inf_scene_obj_ids)}')
        #         if inf_id in ren.models:
        #             raise ValueError(f'Collision of object ids: {obj_id}')
        #         obj_path = inf_obj_path_tmpl.format(obj_id=inf_id)
        #         ren.add_object(inf_id, obj_path)
        ...
        # HACK: Load info about the GT poses (e.g. visibility) for the current scene.
        ...
        for im_ind, (im_id, im_targets) in enumerate(scene_targets.items()):
            ...
            for obj_id, target in im_targets.items():
                ...
                # Get the estimates.
                try:
                    # HACK: cannot assume anymore that we can preselect estimates
                    # based on id since we don't have est-gt object id correspondances
                    # -> instead, select all pose est prosposals for given image
                    # obj_ests = ests_org[scene_id][im_id][obj_id]
                    obj_ests = []
                    for est in ests_org[scene_id][im_id].values():
                        obj_ests += est
                ...
                # HACK: don't do it since we need to evaluate all propositions
                ...
                for est_id, est in obj_ests_sorted:
                    ...
                    t_e = est["t"]
                    s_e = est["scale"]*1000
                    bbox_visib_est = est['bbox_visib'].flatten().tolist()
                    ...
                    for gt_id, gt in enumerate(scene_gt[im_id]):
                        ...
                        # HACK: retrieve groundtruth visible bounding box and compare with estimated one
                        bbox_visib_gt = scene_gt_info[im_id][gt_id]['bbox_visib']  # xm,ym,w,h

                        if not inf_id in models:
                            models[inf_id] = inout.load_mesh(unique_inf_scene_obj_path[inf_id], scale=1.0)

                        # LOAD meshes only when needed
                        if p["error_type"] in ["ad", "add", "adi", "mssd", "mspd", "proj", "chamfer", "chamfer_proj"]:
                            misc.log(f"Loading mesh {inf_id}...")
                            models[inf_id]["pts"] *= s_e

                        # HACK: load object model per scenes
                        if p["error_type"] in ["vsd", "cus"]:
                            misc.log(f"Loading mesh {inf_id} in renderer...")
                            ren.add_object_from_mesh(inf_id, models[inf_id], scale=s_e)

                        loaded_inf_ids.append(inf_id)
                        ...
                        #         )
                        #     )
                        # HACK: bypass sphere_projections_overlap check
                        sphere_projections_overlap = True
                        ...
                        #     )
                        # HACK: bypass spheres_overlap check
                        spheres_overlap = True
                        ...
                        if p["error_type"] == "vsd":
                        ...
                        elif p["error_type"] == "chamfer_proj":
                            e = [
                                pose_error.chamfer_proj(
                                    R_e,
                                    t_e,
                                    R_g,
                                    t_g,
                            ...
                            if sphere_projections_overlap:
                                e = [pose_error.cus(R_e, t_e, R_g, t_g, K, ren, inf_id, obj_id)]
                            else:
                                e = [1.0]
                        ...
                        if p["error_type"] in ["cus"]:
                            ren.remove_object(inf_id)
        ...
        # HACK
        unique_loaded_inf_ids = set(loaded_inf_ids)
        if p["error_type"] in ["ad", "add", "adi", "mssd", "mspd", "proj", "chamfer", "chamfer_proj"]:
            for obj_id in unique_loaded_inf_ids:
                if obj_id in models:
                    del models[obj_id]
        if p["error_type"] in ["vsd", "cus"]:
            for obj_id in unique_loaded_inf_ids:
                if obj_id in ren.models:
                    ren.remove_object(obj_id)
